...
Page({
  ...
  onLoad: async function (options) {
    await this.updateUserInfo(await getUserInfoFromBackend());
  ...
  onShow: async function () {
    await this.updateUserInfo(await getUserInfoFromCache());
  },
  ...
  onShareAppMessage: function () {
    ...
    return {
      path: '/pages/giftExchange/index?' + query
    };
  },

  login: async function () {
    wx.navigateTo({
      url: `/pages/userHome/index?envId=${envList[0].envId}`,
    })
  },
  ...
  updateUserInfo: async function (userInfo) {
    ...
    if (this.data.activityUpdateTimer) {
      clearInterval(this.data.activityUpdateTimer)
      this.setData({
        activityUpdateTimer: null
      })
    }
